...
`git` tracks contents of files as `blob`s, that it distributes later into different directories.

We'll be looking into how `git` creates these directories, why it creates them, and how it distributes its objects in them.
...
`git` stores everything into the `.git/objects` directory, but that directory will only ever contain the following:

* A directory called `info`
* A directory called `pack`
* A directory whose name is going to be a 2-character hexadecimal string

The `info` and `pack` ones are out of scope for this article, but the interesting question would be:

* Why would we only find directories whose name is a 2-character hexadecimal string ?
