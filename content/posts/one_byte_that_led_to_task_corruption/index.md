...
draft = false
title = 'How one single byte led to silent Celery task corruption'
...
we couldn't make sense of, which eventually started bringing down our services.
<!--more-->
...
RabbitMQ speaks AMQP, which is a protocol that handles everything from publishing messages to consuming them.
...
One of those properties is `headers`, which lets you attach arbitrary key-value pairs to a message.

In AMQP terms, that structure is called a **Field Table**, which is really just a binary encoding of a dictionary.
...
That type tag is the crux of everything. AMQP defines a fixed set of value types such as: short int, long string, boolean, decimal, etc, and each one has a single-byte identifier. The decoder reads the tag first, then knows exactly how many bytes to consume for the value.

Section 4.2.1 of the [spec](https://www.rabbitmq.com/resources/specs/amqp0-9-1.pdf) lists all of them.

If you want more details of how the decoding happens, you can dive into the following section that contains a step-by-step
explained example
...
To transfer the messages, we used [`aio-pika`](https://docs.aio-pika.com/) to read from the source queue, tweak the headers and republish to the destination.
...
* `pamqp` sends the following `73 01 2C` byte sequence over the wire:
  * The type tag byte is `73` (hex), which is the ASCII character `s` that stands for a signed 16-bit integer in pamqp's convention
  * Since it's a 16-bit integer, `300` is encoded over the next 2 bytes: `01 2C`

* `py-amqp` receives the same `73 01 2C` byte sequence:
  * The type tag byte is `73`, ASCII character `s`, however this time, in py-amqp's convention, `s` means short string
...
That's exactly what happened to us. The headers on our messages included a `timelimit` key with the value `[None, 300]` where `None` was the soft timeout and`300` was the hard timeout in seconds.
...
So RabbitMQ's broker uses `s` for signed 16-bit. **pamqp deliberately follows this** as stated in its v1.5.0 [changelog](https://gmr.github.io/pamqp/changelog/#150-2014-11-05). It explicitly says it aligned field table type indicators to the RabbitMQ protocol errata.

But when it came for py-amqp, it went the other way and followed the 0-9-1 spec where `s` would stand for a short string and `U` for a signed 16-bit, with no mention of the conflict anywhere in its codebase.
...
We swapped `aio-pika` out for `pika` to do the message transfer. `pika` has its own wire encoder and uses the `U` type tag for 16-bit integers, aligning with the same convention py-amqp expects on the receiving end, so the two sides finally agreed. Problem solved for new messages.
...
* Write a script to read from those DLQs, check the `timelimit` header, and fix the value from `[None, ',']` to `[None, 300]`
* Redrive those messages back to the original destination queue

---

While debugging this, I would have loved to find something that connected all the pieces, except that I didn't.

And that's what has pushed me to write this article, so that the next person who ends up staring at a comma that should've
been an integer for a timeout value would know where it came from.
